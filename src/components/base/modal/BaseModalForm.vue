	...
	<div class="modal" v-show="isShow" @click="modalClick">
		<div class="form-wrapper animate">
			...
			<div class="form-content">
				...
				<div class="form-input-info" v-if="gotDetail" tabindex="0">
					...
					<div class="input-wrapper" v-if="gotDetail">
						...
						<ComboBox
							ref="Gender"
							style="height: 40px"
							:comboBoxData="selectBoxesData[3]"
							:idField="selectBoxesData[3][1]"
							:nameField="selectBoxesData[3][2]"
...
export default {
	name: "ModalForm",
	components: { SelectBox, BaseInput, CurrencyInput, Dialog, ComboBox },
	props: {
	...
	methods: {
		...
		saveModalForm() {
			...
			// this.formData["WorkStatus"] = parseInt(this.formData["WorkStatus"]);
			this.dialog.isShow = false;
			var dataPost = JSON.stringify(this.formData);
			// console.log(dataPost);
			if (this.formMode == 0) {
				this.axios
					.post("https://localhost:44368/api/v1/Employees", dataPost, {
						headers: {
				...
				this.axios
					.put(
						`https://localhost:44368/api/v1/Employees/${this.formDataId}`,
						dataPost,
						{
							headers: {
								// Overwrite Axios's automatically set Content-Type
		...
		focusForm(){
			console.log(this.$el.querySelector(".form-input-info").focus());
		},
		getDetail: async function() {
			await this.axios
				.get(`https://localhost:44368/api/v1/Employees/${this.formDataId}`)
				.then(res => {
					if (res.status == 200) {
		...
		getNewCode: async function(codeField) {
			await this.axios
				.get("https://localhost:44368/api/v1/Employees/NewCode")
				.then(res => {
